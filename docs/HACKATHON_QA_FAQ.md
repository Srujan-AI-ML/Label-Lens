...
### Q9: How are Microsoft Word (.DOCX) and PDF reports generated, and how do you guarantee data consistency between formats?
**Answer:** We implemented a unified reporting engine in `reportService.ts`. Instead of separate disconnected generators, a shared data transformation layer (`buildUnifiedReportData`) produces the canonical audit checklist, scoring matrix, and violation logs. This shared object powers both `jsPDF` for digital certificates and the `docx` library for genuine, editable Word documents (`.docx`). Furthermore, inspectors can export the complete product registry dataset as a landscape Word table with repeated headers and complete regulatory metadata.

### Q10: How does the system handle diverse regulatory frameworks across multiple industries (e.g. FSSAI for food vs BIS for electronics vs CDSCO for medical devices)?
**Answer:** India enforces sector-specific statutory declarations in addition to general Legal Metrology Rules. Our category-aware engine in `categoryRequirements.ts` dynamically configures the verification matrix across 8 distinct commodity categories. For example, selecting "Food & Beverage" enforces a 14-digit FSSAI license; "Electrical / Electronics" checks BIS CRS registration; "Toys" validates BIS Toy Safety standards; and "Medical Devices" inspects CDSCO registration—ensuring accurate scoring without penalizing non-food products for missing food licenses.
